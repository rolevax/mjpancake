...
Grid {
    ...
    GomboToggle {
        model: [ "东风", "东南" ]
        onActivated: function(index) { gameRule.roundLimit = (index + 1) * 4; }
        Component.onCompleted: { currentIndex = gameRule.roundLimit / 4 - 1; }
    }

    GomboToggle {
        model: [ "击飞 X", "击飞 O" ]
        onActivated: function(index) { gameRule.fly = !!index; }
        Component.onCompleted: { currentIndex = gameRule.fly; }
    }

    GomboToggle {
        model: [ 30000, 100000 ]
        onActivated: function(index) { gameRule.returnLevel = model[index]; }
    }

    GomboToggle {
        model: [ "丘20", "丘0" ]
        onActivated: function(index) { gameRule.hill = [ 20000, 0 ][index]; }
    }

    GomboToggle {
        model: [ "2和/3流", "头跳" ]
        onActivated: function(index) { gameRule.headJump = !!index; }
    ...
    GomboToggle {
        model: [ "杠宝牌 X", "杠宝牌 O" ]
        onActivated: function(index) { gameRule.kandora = !!index; }
        Component.onCompleted: { currentIndex = gameRule.kandora; }
    }

    GomboToggle {
        model: [ "赤0", "赤3", "赤4" ]
        onActivated: function(index) { gameRule.akadora = index; }
        Component.onCompleted: { currentIndex = gameRule.akadora; }
    }

    GomboToggle {
        model: [ "荒牌满贯 X", "荒牌满贯 O" ]
        onActivated: function(index) { gameRule.nagashimangan = !!index; }
        Component.onCompleted: { currentIndex = gameRule.nagashimangan; }
    }

    GomboToggle {
        model: [ "一发 X", "一发 O" ]
        onActivated: function(index) { gameRule.ippatsu = !!index; }
        Component.onCompleted: { currentIndex = gameRule.ippatsu; }
    }

    GomboToggle {
        model: [ "包杠 X", "包杠 O" ]
        onActivated: function(index) { gameRule.daiminkanPao = !!index; }
